...
class Dashboard extends CFormModel
{
    ...
    public function saleDailyChart()
    {
        ...
        $sql="SELECT DATE_FORMAT(s.sale_time,'%d/%m/%y') DATE,currency_code,SUM(sub_total) sub_total,SUM(total) total
              FROM v_sale_invoice s
              WHERE ( s.sale_time BETWEEN DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW() )
              AND s.status=:status
              GROUP BY DATE_FORMAT(s.sale_time,'%d/%m/%y'),currency_code";
