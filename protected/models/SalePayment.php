...
class SalePayment extends CActiveRecord
{
    ...
    */
    ...
    public function invoiceRawData($client_id, $compare_operator)
    {
        $sql = $this->invoiceQuery($compare_operator);

        return Yii::app()->db->createCommand($sql)->queryAll(true, array(':client_id' => $client_id));

    }

    public function singleInvoiceRawData($sale_id, $client_id, $compare_operator)
    {
        $sql = $this->singleInvoiceQuery($compare_operator);

        return Yii::app()->db->createCommand($sql)->queryAll(true, array(':sale_id' => $sale_id,':client_id' => $client_id));
    ...
    public function invoice($client_id, $compare_operator)
    {
        ...
        $rawData = $this->invoiceRawData($client_id,$compare_operator);
    ...
    public function invoicePerCurrency($client_id, $currency_code, $compare_operator)
    {
        $sql = "SELECT sale_id,sale_time,account_name client_name,amount sub_total,0 discount_amount,payment_amount paid,balance
        ...
        $rawData = Yii::app()->db->createCommand($sql)->queryAll(true, array(':client_id' => $client_id, ':currency_code' => $currency_code));
