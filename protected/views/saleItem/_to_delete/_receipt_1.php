            ...
            <tbody>
                ...
                <?php endforeach; ?> <!--/endforeach-->  
                    <tr class="gift_receipt_element">
                        ...
                        <td colspan="1" style='text-align:right;border-top:2px solid #000000;'> <?php echo Yii::app()->settings->get('site', 'currencySymbol') . number_format($sub_total,Common::getDecimalPlace(), '.', ','); ?></td>
                    </tr>
                    <tr class="gift_receipt_element">
                        <td colspan="3" class="text-right">Total</td>
                        <td colspan="1" class="text-right"><?php echo TbHtml::encode(Yii::app()->settings->get('site', 'currencySymbol') . number_format($total,Common::getDecimalPlace(), '.', ',')); ?></td>
                    </tr>    
                    <?php foreach($payments as $payment_id=>$payment): ?> 
                    <tr class="gift_receipt_element">
                        <td colspan="3" class="text-right">Payment</td>
                        <td colspan="1" class="text-right"> <?php echo TbHtml::encode(Yii::app()->settings->get('site', 'currencySymbol') . number_format($payment['payment_amount'],Common::getDecimalPlace(), '.', ',')); ?> </td>
                    </tr>
                    <?php endforeach;?>
                    <tr class="gift_receipt_element">
                        <td colspan="3" class="text-right">Change Due</td>
                        <td colspan="1" class="text-right"> <?php echo TbHtml::encode(Yii::app()->settings->get('site', 'currencySymbol') . number_format($amount_change,Common::getDecimalPlace(), '.', ',')); ?> </td>
                    </tr>
            </tbody>
        </table>
