...
<div class="container"> 
            ...
            <tbody>
                ...
                <?php endforeach; ?> <!--/endforeach-->  
                    <tr class="gift_receipt_element">
                        ...
                        <td colspan="1" style='text-align:right;border-top:2px solid #000000;'> <?php echo number_format($sub_total,Common::getDecimalPlace(), '.', ','); ?></td>
                    </tr>
                    <tr class="gift_receipt_element">
                        <td colspan="3" class="text-right">Total</td>
                        <td colspan="1" class="text-right"><?php echo TbHtml::encode(number_format($total,Common::getDecimalPlace(), '.', ',')); ?></td>
                    </tr>    
                    <?php foreach($payments as $payment_id=>$payment): ?> 
                    <tr class="gift_receipt_element">
                        <td colspan="3" class="text-right">Payment</td>
                        <td colspan="1" class="text-right"> <?php echo TbHtml::encode(number_format($payment['payment_amount'],Common::getDecimalPlace(), '.', ',')); ?> </td>
                    </tr>
                    <?php endforeach;?>
            </tbody>
        </table>
    ...
    <div class="row text-right">
        ...
        <div class="col-xs-3">
            <p>
                <?php echo number_format($sub_total,Common::getDecimalPlace(), '.', ','); ?><br>
                <?php if ($total_discount!==NULL && $total_discount>0) { ?> <?php echo '%' . $total_discount; ?> <br> <?php } ?>
                <?php echo TbHtml::encode(number_format($total,Common::getDecimalPlace(), '.', ',')); ?> <br>
                <?php foreach($payments as $payment_id=>$payment): ?> 
                    <?php echo TbHtml::encode(number_format($payment['payment_amount'],Common::getDecimalPlace(), '.', ',')); ?> <br>
                 <?php endforeach;?>
                <?php echo TbHtml::encode(number_format($amount_change,Common::getDecimalPlace(), '.', ',')); ?> <br>
